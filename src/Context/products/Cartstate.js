...
export default function Cartstate(props) {
    ...
    const host = "http://localhost:5001/api/user/cart"
...
const addtocart =async (title,price,quantity=1)=>{
 ...
 //............................fetch OrderList.................
  const fetchorderlist =async ()=>{
    const response = await fetch(`http://localhost:5001/api/user/orderdetail/fetchorderlist`, {
      method: 'GET',
      headers: {
        'auth-token':localStorage.getItem("token")
      }
  ...
  const addtoorder =async (orderId,total,date)=>{
    const response = await fetch(`http://localhost:5001/api/user/orderdetail/addtoorder`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'auth-token':localStorage.getItem("token")
